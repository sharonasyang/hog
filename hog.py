...
def picky_piggy(score):
    ...
    if score == 0:
        ...
        if rem == 0:
            rem = 6
        return math.trunc((1 / 7) * pow (10, rem)) % 10
    # END PROBLEM 2
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while max(score0, score1) < goal:
        if who == 0:
            ...
            """
            The reason I split the code into two lines is because score0 in the second line depends on the
            outcome of score0 in the first line. If the two lines need to be combined as one, I think 
            it would look like this: 
            score0 = (score0 + take_turn(strategy0(score0, score1), score1, dice, goal)) 
                   + hog_pile(score0 + take_turn(strategy0(score0, score1), score1, dice, goal), score1)
            As you can see, take_turns() would need to be called twice, which would decrease the performance of 
            the code. Combining the two lines of code into one line would also decrease its readability.
            """        
        else:
            score1 += take_turn(strategy1(score1, score0), score0, dice, goal)
            score1 += hog_pile(score1, score0)
            #score1 = (score1 + take_turn(strategy1(score1, score0), score0, dice, goal)) + hog_pile(score1 + take_turn(strategy1(score1, score0), score0, dice, goal), score0)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7 
    prev_score, highest_diff = last_score, running_high
    ...
    def say(score0, score1):
        if who == 0:
            ...
            if diff > highest_diff:
                print (diff, "point(s)! That's a record gain for Player 0!")
        elif who == 1:
            diff = score1 - prev_score
            last_score = score1
            if diff > highest_diff:
                print (diff, "point(s)! That's a record gain for Player 1!")
        return announce_highest(who, last_score, max(highest_diff, diff))
    return say    
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    calculate_avg = make_averaged(roll_dice, trials_count)

    for num_rolls in range(1, 11, 1):
        avg_score = calculate_avg(num_rolls, dice)
        if avg_score > max_avg:
            max_avg, max_num_rolls = avg_score, num_rolls
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    # BEGIN PROBLEM 12
    num_rolls = 6
    """if(score + picky_piggy(opponent_score) <= opponent_score + 1 and score + picky_piggy(opponent_score) >= opponent_score - 1):
        return num_rolls
    return hog_pile_strategy(score, opponent_score, 3)"""
    if score + picky_piggy(opponent_score) <= opponent_score + 1 and score + picky_piggy(opponent_score) >= opponent_score - 1:
        return num_rolls
    return picky_piggy_strategy(score, opponent_score, 3)
    # END PROBLEM 12
